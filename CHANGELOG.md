...
- **`ArduinoManager` 연결 끊김 감지 및 자동 재연결(`AutoReconnect`) 지원(`Runtime/Hardware`):** `OnConnected`, `OnDisconnected` R3 스트림 추가. 시리얼 읽기 예외(케이블 분리 등) 발생 시 `AutoReconnectIntervalMs` 주기로 백그라운드 재연결 자동 수행(사용자의 명시적 `Disconnect()` 호출 시에는 재연결 루틴 안전 취소).
...
- **`SoundManager` 볼륨 제어 및 음소거 지원(`Runtime/UI`):** `MasterVolume`, `BGMVolume`, `SFXVolume`, `IsMuted`, `SetMasterVolume`, `SetBGMVolume`, `SetSFXVolume`, `SetMute`, `ToggleMute` API 추가. BGM 및 SFX 재생 시 세부 볼륨 배율 및 음소거 상태 즉시 반영.
- **`FadeManager` 페이드 색상 커스텀 및 SortingOrder 설정 지원(`Runtime/UI`):** `FadeInAsync`/`FadeOutAsync`에 `Color? color = null` 지원(기본값 검정색). Canvas 렌더링 우선순위를 제어할 수 있는 `SortingOrder` 프로퍼티 추가.
...
### Changed
- **매니저 계층 8종의 `DontDestroyOnLoad` 중복 생성 방어 일원화:** `FadeManager`, `UIManager`, `SoundManager`, `VideoManager`, `InactivityTimer`, `ShutdownScheduler`, `ApiManagerBase`, `ArduinoManager`에 `_isInstantiated` 및 `_isOriginal` 정적 플래그 패턴을 일원화 적용. 씬 재로드 시 인스턴스 무한 증식을 방지하고 Domain Reload 비활성화 환경에서도 안전하게 동작하도록 개선.
