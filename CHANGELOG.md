...
### Fixed
- **`InactivityTimerTests`가 실제 입력에 따라 간헐적으로 실패하던 문제 수정(`Tests/Runtime`):** `InactivityTimer`는 `InputSystem.onEvent`로 모든 장치의 입력을 받아 타이머를 초기화하는데, 테스트가 실제 입력 환경에서 돌아 실행 중 마우스·키보드 입력이 들어오면 타이머가 초기화되어 결과가 흔들렸음(예: 타임아웃 뒤 입력으로 이벤트가 다시 발동해 '재입력 없이 다시 발동함' 실패). Input System의 `InputTestFixture`로 실제 장치 입력을 차단한 가상 입력 환경에서 실행하도록 바꾸고, 테스트 어셈블리에 `Unity.InputSystem`·`Unity.InputSystem.TestFramework` 참조를 추가함. 이전에는 실제 입력 때문에 검증할 수 없던 '입력이 들어오면 발동이 늦춰진다'를 가상 키보드로 검증하는 테스트 1건을 추가함(입력 구독을 빼면 실패함을 확인). 런타임 동작 변화는 없음.
