...
- **오래된 로그 파일 자동 정리(보관 기간 30일) 추가:** ZLogger 2.5.10의 rolling provider는 파일을 새로 만들 뿐 오래된 파일을 지우지 않아(보관 개수/기간 옵션 미지원), 회전만으로는 날짜별 파일이 계속 쌓여 디렉터리 총량이 여전히 무한정 커짐. 이를 보완하기 위해 마지막 기록 시각이 30일을 지난 `GameLog_*.txt`를 삭제하는 `LogRetentionService`를 추가함. **시작 시 1회 정리 후 24시간마다 반복**하여, 재시작 없이 장기간 도는 경우에도 디스크 사용량이 최근 30일치로 유지됨. 정리 실패는 앱 실행을 막지 않도록 경고 로그만 남기고 무시하며, 서비스 수명은 `Application.exitCancellationToken`에 묶여 앱 종료 시 정리됨. (에디터/WebGL은 기존과 동일하게 파일 로깅 대상에서 제외)
